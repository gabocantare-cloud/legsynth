...
python scripts/reproduce.py             # every number and figure below
```

Every number below is generated by that command, except the four §7 robustness numbers and
the clearance-fit figures, which come from `python scripts/reproduce.py --with-studies`
(46 optimization campaigns; the studies alone measured 2 h 32 m on 12 workers) and are
labelled where they appear. None is quoted from the paper except where it is explicitly
...
against the integrated form at twenty seed triples: the paired difference in best achievable
gait error averages +0.0004, 95% CI [−0.011, +0.012], paired *t*-test *p* = 0.95. No detectable
effect — which is not proof that the bias cancels, only that this experiment cannot see it
move. Numbers in
...
| `robustness.py` | the four studies behind §7 of the results | 46 campaigns, 2 h 32 m measured on 12 workers |
