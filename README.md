...
```
pip install -e . && pytest              # 90 tests
python scripts/reproduce.py             # every number and figure below, ~10 min
```

Every number below is generated by that command. None is quoted from the paper except where
it is explicitly labelled as the paper's.
...
`reproduce.py` runs the four stages below in the order they depend on each other. Each is
